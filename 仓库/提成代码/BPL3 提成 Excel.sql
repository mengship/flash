            ...
            SELECT
                ...
                case
                    ...
                    AND (UNIX_TIMESTAMP(上班打卡时间) - UNIX_TIMESTAMP(旷工最晚时间)) / 60 > 0 then 0
                        ...
                        ,
                case
                    when 应出勤 = 1 AND ( 上班打卡时间 is null or 下班打卡时间 is null ) AND 请假 = 0 then 1
                                ...
                                (
                                    SELECT
                                        ...
                                        if(
                                            ...
                                            1,
                                            0
                                        ) 应出勤 -- ,if(ad.PH=0 AND ad.OFF=0 AND ad.attendance_time>0,1,0) 出勤天数
                                        ,
                                        case
                                            when ad.PH = 0
                                            AND ad.OFF = 0
                                        ...
                                        end 出勤 -- ,if(ad.PH=0 AND ad.OFF=0 AND ad.attendance_time=0,1,0) 未出勤天数
                                        ,
                                        case
                                            when ad.PH = 0
                                            AND ad.OFF = 0
                                        ...
                                        ad.`AB` -- ,UNIX_TIMESTAMP(ad.`attendance_started_at`)-UNIX_TIMESTAMP(concat(ad.`stat_date`,' ',ad.`shift_start`,':','00' )) 迟到分钟
                                        ...
                                        ,
                                        case
                                            when ad.PH = 0 AND ad.OFF = 0 AND ( ad.`attendance_started_at` is null or ad.`attendance_end_at` is null ) AND ad.leave_type not in (1, 2, 12, 3, 18, 4, 5, 17, 7, 10, 16, 19) then 1
                                            when ad.PH = 0 AND ad.OFF = 0 AND UNIX_TIMESTAMP(ad.`attendance_started_at`) - UNIX_TIMESTAMP( concat(ad.`stat_date`, ' ', ad.`shift_start`, ':', '00') ) > 0 AND UNIX_TIMESTAMP(ad.`attendance_started_at`) - UNIX_TIMESTAMP( concat(ad.`stat_date`, ' ', ad.`shift_start`, ':', '00') ) <= 30 then 0 -- when ad.PH=0 AND ad.OFF=0
                                    ...
                                    WHERE
                                        sd.`一级部门` = 'Thailand Fulfillment' --                     AND left(ad.`stat_date`,7)='2023-07'
                                        AND ad.`stat_date` BETWEEN date_sub(
                                            date_sub(
...
SELECT
    ...
    null B2B,
    工作量 计提件数,
    round(基础提成2 * 考勤系数 - 迟到 * 5 + 全勤奖, 2) 工作量提成,
    round(基础提成2 * 考勤系数 - 迟到 * 5 + 全勤奖, 2) 提成,
    null 业务罚款,
    null 现场管理罚款,
    迟到扣款 出勤罚款,
    全勤奖,
    ...
    (
        SELECT
            ...
            IF(
                ...
                ) over (PARTITION by 'BPL3'),
                基础提成
            ) 基础提成2
        FROM
            (
                SELECT
                    a.* --         ,调整超额提成*考勤系数-迟到*5 考勤提成
,
                    迟到 * 5 迟到扣款,
                    if(
                        职位 = 'Inbound Supervisor',
                        1.3 * AVG(
                            if(
                                职位 = 'Inbound'
                                and 是否当月入职 = '非当月入职',
                ...
                FROM
                    (
                        SELECT
                            ...
                            IF(
                                是否当月入职 = '非当月入职',
                                (工作量 - 调整目标值) * 超额系数 * 出勤 / 部门出勤,
                                ((工作量 - 调整目标值) * 超额系数 * 出勤 / 部门出勤) -((工作量 - 调整目标值) * 超额系数 / 部门出勤) * 7
                            ) 调整超额提成
                        FROM
                            (
                                SELECT
                                    ...
                                    if(
                                        职位 = 'Inbound',
                                        实际上架数量,
                                        if(职位 in ('Pack', 'Pick', 'Outbound'), 订单量, null)
                                    ) 工作量,
                                    订单量,
                                    实际上架数量,
                                    ...
                                    (
                                        SELECT
                                            ...
                                            IF(
                                                职位 = 'Inbound',
                                                人效 * 2 * 26,
                                                IF(职位 in ('Pack', 'Pick', 'Outbound'), 人效 * 16 * 26, null)
                                            ) 目标值 # @TODO inbound 3 人, 'Pack','Pick','Outbound'共 18 人,需要进行调整
,
                                            超额系数,
                                            应出勤,
                                            出勤,
                                            ...
                                            hsi.hire_date 入职日期,
                                            IF(
                                                LEFT(hsi.hire_date, 7) = LEFT(date(NOW() - INTERVAL 1 hour), 7),
                                                '当月入职',
                                                '非当月入职'
                                            ) 是否当月入职,
                                            if(
                                                职位 = 'Inbound',
                                                SUM(if(职位 = 'Inbound', 应出勤, 0)) over (PARTITION by 'BPL3'),
                                                IF(
                                                    职位 in ('Pack', 'Pick', 'Outbound'),
                                                    SUM(if(职位 in ('Pack', 'Pick', 'Outbound'), 应出勤, 0)) over (PARTITION by 'BPL3'),
                                                    null
                                                )
                                            ) 部门应出勤,
                                            if(
                                                职位 = 'Inbound',
                                                SUM(if(职位 = 'Inbound', 出勤, 0)) over (PARTITION by 'BPL3'),
                                                IF(
                                                    职位 in ('Pack', 'Pick', 'Outbound'),
                                                    SUM(if(职位 in ('Pack', 'Pick', 'Outbound'), 出勤, 0)) over (PARTITION by 'BPL3'),
                                                    null
                                                )
                                            ) 部门出勤,
                                            tt.kpi
                                        FROM
                                            tmpale.tmp_th_ffm_BPL3_stat2 tt #@TODO 改配载表
                                    ...
                                    LEFT join (
                                        ...
                                        from
                                            test.th_ffm_outbound_total_detail_l t
                                        where
                                            date(t.打包完成时间) BETWEEN date_sub(
                                                date_sub(
                                                    date_format(now(), '%y-%m-%d'),
                                                    interval extract(
                                                        day
                                    ...
                                    left join (
                                        ...
                                        from
                                            (
                                                ...
                                                where
                                                    vp.physicalwarehouse_name = 'BPL3'
                                                    and o.status = 30
