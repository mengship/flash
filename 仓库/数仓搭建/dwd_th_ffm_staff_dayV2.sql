...
-- 人力信息表
drop table if exists dwm.dwd_th_ffm_staff_dayV2;
create table dwm.dwd_th_ffm_staff_dayV2 as
-- delete from dwm.dwd_th_ffm_staff_dayV2 where 统计日期 >= date_sub(date(now() + interval -1 hour),interval 100 day); -- 先删除数据
-- insert into dwm.dwd_th_ffm_staff_dayV2 -- 再插入数据
SELECT 
    人员信息
    ,统计日期
    ,一级部门
        ...
        (
            ...
            (
                ...
                WHERE sd.`一级部门`='Thailand Fulfillment'
                    -- AND ad.`stat_date`>= date_sub(date(now() + interval -1 hour),interval 100 day)
                    AND ad.`stat_date`>= '2023-12-01'
            ) ad 
        ) ad 
    ) ad 
) ad 
