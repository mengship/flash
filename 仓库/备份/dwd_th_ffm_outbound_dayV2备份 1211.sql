...
,warehouse_detailname
       ...
       select
        ...
        ,do2.out_operator
        ,do2.packtimetype
        -- ,if(led.warehouse is not null and outtimetype='nooutbound intime', '快递迟到揽收', do2.outtimetype) outtimetype
        ...
        (
            SELECT 
                ...
                ,case when warehouse_name='AutoWarehouse'   then 'AGV'
                    when warehouse_name='AutoWarehouse-人工仓' then 'AGV-人工仓'
                    when warehouse_name='BPL-Return Warehouse'  then 'BPL-Return'
                    when warehouse_name in ('BPL3-LIVESTREAM', 'BPL3- Bangphli3 Livestream Warehouse') then 'BPL3'
                    when warehouse_name='BangsaoThong'  then 'BST'
                    when warehouse_name IN ('BKK-WH-LAS2电商仓')    then 'LAS'
                    end warehouse_detailname
            ...
            (
                select
                    ...
                    ,case when do.is_tiktok is not null then 'Tik Tok'
                        ...
                        else platform_source end as platform_source
                    ,do.seller_name
                    ,do.seller_id
                    ,do.created_time
                    ,do.created_date
    ...
    left join (select delivery_sn from dwm.tmp_th_ffm_notimesn_detail) nd on do2.delivery_sn = nd.delivery_sn
