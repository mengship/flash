...
(
    ...
    where 1=1
        and sgss.`date` > '2024-10-01'
        and sgss.`date` < CURRENT_DATE
        and s.name='FFM-TH'
    group by 1,2,3,4
...
union all -- 入库
    ...
    (
        ...
        left join wms_production.seller s on s.id = an.seller_id
            ...
            and an.complete_time < current_date
            and s.name='FFM-TH'
...
union all  -- 包材出库
    ...
    select
        '包材出库'
        ,仓库名称 仓库
        ,verify_date + interval -1 day verify_date
        ,bar_code 商品条码
        ,sum(out_num) out_num
        ,null
    from
    (
        ...
        where 1=1
            and rw.verify_time >= '2024-10-02'
            and rw.verify_time < CURRENT_DATE
            and rw.type=3
            and s.name='FFM-TH'
    ) t0
    group by 1,2,3,4
    ...
    (
        select
            ...
            ,case when w.name='AutoWarehouse' then 'AGV'
                ...
                when w.name IN ('BKK-WH-LAS2电商仓')         then 'LAS' # ,'PMD-WH','BKK-WH-Ecommerce','BKK-WH-LAS物料仓'
                when w.name='LCP Warehouse' then 'LCP' end 仓库名称
            ,case when rw.type=2 then '调拨出库'
                when rw.type=1 then '出库'
                end as title
            ,rw.out_warehouse_time
            ,rwg.out_num
            ,sg.bar_code
...
union all  -- 收费包材明细数据
    ...
    (
        ...
        where b.documents = 6
        and a.business_date >= '2024-10-01'
