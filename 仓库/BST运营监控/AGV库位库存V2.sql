SELECT 
        ...
        ,sum(if(t1.'库位编码' like 'H%',t1.'体积',0))                                 '高位货架库容-规划'
        ,sum(if(((t1.'库位编码' like 'H%') and t1.'正品库存' > 0), t1.'商品体积', 0))     '高位货架商品体积' #使用 

        ,'1-2'
        ,count(DISTINCT (if(t1.'库位编码' like 'FA-B%', t1.'库位编码',0)))  -1             '轻型货架库位-规划'
        ,sum(if(((t1.'库位编码' like 'FA-B%') and t1.'正品库存' > 0 ), 1, 0))                 '轻型货架库位' #使用
        ,sum(if(t1.'库位编码' like 'FA-B%',t1.'体积',0))                                '轻型货架库容-规划'
        ,sum(if(((t1.'库位编码' like 'FA-B%') and t1.'正品库存' > 0), t1.'商品体积', 0))     '轻型货架商品体积' #使用
