...
(
    ...
    (
        SELECT
            ...
            ,oobt.operator
            ,bau.user_name
            ,date(oobt.gmt_modified + interval -1 hour) now_date
            ,hour(oobt.gmt_modified + interval -1 hour) now_hour
    ...
    (
        SELECT
            ...
            ,date(wsg.operation_time + interval -1 hour) now_date
            ,hour(wsg.operation_time + interval -1 hour) now_hour
    ...
    (
        select
            ...
            ,picker_id
            ,real_name
            ,date(complete_time + interval -1 hour) now_date
            ,hour(complete_time + interval -1 hour) now_hour
            ,sum(picknum) picknum
        from
        (
            select 
