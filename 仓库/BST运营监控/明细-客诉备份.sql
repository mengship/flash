...
AND left(wo.created,10) >=left(now()- INTERVAL 70 day,10))

SELECT 创建日期,仓库名称,'有责投诉' type,sum(CASE WHEN 仓责判断='有责投诉' THEN 1 ELSE 0 end)客诉量 FROM KS WHERE 仓库名称 IS NOT null
GROUP BY 1,2,3 
UNION
SELECT 创建日期,仓库名称,'无责投诉' type,sum(CASE WHEN 仓责判断='无责投诉' THEN 1 ELSE 0 END )客诉量 FROM KS WHERE 仓库名称 IS NOT null
GROUP BY 1,2,3
UNION
SELECT 创建日期,仓库名称,'及时关闭工单' type,sum(是否及时)客诉量 FROM KS WHERE 仓库名称 IS NOT NULL
GROUP BY 1,2,3
