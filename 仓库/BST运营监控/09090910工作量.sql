...
(
    ...
    (
        select 
            ...
            ,hour(do.delivery_time) now_hour
        ...
        where 1=1
            ...
            and 'B2C'=do.TYPE
            and date(do.delivery_time) >= '2024-09-09'
            and date(do.delivery_time) <= '2024-09-10'
        group by 1,2,3,4,5
    ) out
    ...
    (
        select
            ...
            ,picker_id
            ,real_name
            ,date(complete_time ) now_date
            ,hour(complete_time ) now_hour
            ,sum(picknum) picknum
        from
        (
            select 
            ...
            where 1=1
                /* and date(po.complete_time + interval -1 hour) >= CURRENT_DATE */
                and date(po.complete_time + interval -1 hour) >= '2024-09-09'
                and date(po.complete_time + interval -1 hour) <= '2024-09-10'
    ...
    (
        ...
        (
                ...
                from
                    ...
                    where 1=1 -- date(db.created)>= 
                    -- date(db.created + interval -1 hour) >= CURRENT_DATE
                    and date(db.created + interval -1 hour)>='2024-09-09' 
                    and date(db.created + interval -1 hour)<='2024-09-10' 
                group by
                    delivery_order_id,
                    db.warehouse_id,
                    db.pack_id,
